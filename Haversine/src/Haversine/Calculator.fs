...
let sumHaversineDistances earthRadius haversine (pairs: Coordinates[]) =
    ...
    for { x0 = x0; y0 = y0; x1 = x1; y1 = y1 } in pairs do
        let dist = haversine x0 y0 x1 y1 earthRadius
...
let customHaversine (x0: float) (y0: float) (x1: float) (y1: float) (earthRadius: float) =
    ...
    let a =
        square (CustomMath.sin (dLat / 2.0))
        + CustomMath.cos lat1 * CustomMath.cos lat2 * square (CustomMath.sin (dLon / 2.0))

    let c = 2.0 * CustomMath.asin (CustomMath.sqrt a)
